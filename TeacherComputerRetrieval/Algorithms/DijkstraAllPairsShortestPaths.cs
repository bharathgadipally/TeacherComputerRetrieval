...
namespace TeacherComputerRetrieval.Algorithms
{
        ...
        where TVertex : IComparable<TVertex>
    {
        ...
        /// Returns the distance between source vertex and destination vertex.
        /// </summary>
        public long ShortestCyclicDistance(TVertex source)
        {
            if (!_allPairsDjkstra.ContainsKey(source))
                throw new Exception($"Source Vertex {source} don't belong to Graph.");

            long shortestPath = 0;
            foreach (var neighbour in _sourceGraph.Neighbours(source))
            {
                if (_allPairsDjkstra[neighbour].HasPathTo(source))
                {
                    var sdSourceToNeighbour = _allPairsDjkstra[source].DistanceTo(neighbour);
                    var sdNeighbourToSource = _allPairsDjkstra[neighbour].DistanceTo(source);

                    if (shortestPath < sdSourceToNeighbour + sdNeighbourToSource)
                    {
                        shortestPath = sdSourceToNeighbour + sdNeighbourToSource;
                    }
                }
            }
            return shortestPath;
